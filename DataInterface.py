...
class DataInterface:
    ...
    def stuAdd(self, name, header, value=None):
        student = findStudent(name)
        if(student.find(header) == None):
            if(value != None): SubElement(student, header).attrib["info"] = value
            else: SubElement(student, header)
        else:
            if(value != None): SubElement(student, header + "1").attrib["info"] = value
            else: SubElement(student, header + "1")

    def stuMassAdd(self, header, value=None):
        students = self.data.find("Students")
        list = students.getchildren()

        for x in range(0, len(list)):
            if(value != None): SubElement(list[x], header).attrib["info"] = value
            else: SubElement(list[x], header)
    ...
    def stuQuery(self, header):
        ...
        for x in range(0, len(list)):
            if(list[x].find(header) != None): canAdd = false
